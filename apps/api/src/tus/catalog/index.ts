...
export type TipoPublicacion =
  (typeof MARKETPLACE_LISTING_KINDS)[keyof typeof MARKETPLACE_LISTING_KINDS]
...
export type MarketplaceBookingMode =
  (typeof MARKETPLACE_BOOKING_MODES)[keyof typeof MARKETPLACE_BOOKING_MODES]
...
export type MarketplaceOutboxEventType =
  (typeof MARKETPLACE_OUTBOX_EVENT_TYPES)[keyof typeof MARKETPLACE_OUTBOX_EVENT_TYPES]
...
export interface MarketplaceCalendarResolver {
  findPrimaryCalendar(
    tenantId: string,
    prestadorId: string
  ): Promise<{ calendarId: string; status: 'active' | 'inactive' } | null>
}
...
export interface MarketplaceStorePort {
  ...
  audit: {
    ...
    list(
      tenantId: string
    ): RegistroAuditoriaMercadoServicios[] | Promise<RegistroAuditoriaMercadoServicios[]>
  }
  idempotency: {
    claim(input: {
      tenantId: string
...
export class InMemoryMarketplaceStore implements MarketplaceStorePort {
  ...
  private readonly idempotencyRecords = new Map<
    string,
    { requestHash: string; response?: MarketplaceCheckoutResponse }
  >()
  private transactionTail: Promise<void> = Promise.resolve()

  readonly merchant = {
    save: async (profile: PerfilPrestador) => {
      this.merchantRecords.set(profile.tenantId, structuredClone(profile))
    },
  ...
  readonly listings = {
    save: async (listing: Publicacion) => {
      this.listingRecords.set(listing.listingId, structuredClone(listing))
    },
    find: async (listingId: string) =>
      this.listingRecords.has(listingId)
        ? structuredClone(this.listingRecords.get(listingId)!)
        : null,
    published: async () =>
    ...
    reserveProduct: async ({
      ...
      if (
        ...
        !listing.published ||
        listing.kind !== MARKETPLACE_LISTING_KINDS.PRODUCT ||
        listing.availabilityVersion !== availabilityVersion ||
        (listing.stock ?? 0) < quantity
      )
        return false
      this.listingRecords.set(listingId, {
        ...listing,
        stock: (listing.stock ?? 0) - quantity,
        availabilityVersion: availabilityVersion + 1,
        updatedAt,
      })
      return true
    },
  ...
  readonly commitments = {
    ...
    forListing: async (listingId: string) =>
      [...this.commitmentRecords.values()]
        .filter((commitment) => commitment.listingId === listingId)
        .map((commitment) => structuredClone(commitment)),
  }

  readonly audit = {
    append: async (records: readonly RegistroAuditoriaMercadoServicios[]) =>
      records.forEach((record) => this.audits.set(record.auditId, structuredClone(record))),
  ...
  readonly idempotency = {
    claim: async ({
      ...
      return record.response
        ? { status: 'replay' as const, response: structuredClone(record.response) }
    ...
    release: async ({ tenantId, key }: { tenantId: string; key: string }) => {
      this.idempotencyRecords.delete(`${tenantId}:${key}`)
    },
  }
  ...
  private snapshot() {
    return {
      merchants: new Map(
        [...this.merchantRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
      listings: new Map(
        [...this.listingRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
      commitments: new Map(
        [...this.commitmentRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
      ...
      idempotency: new Map(
        [...this.idempotencyRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
    }
  }
...
export class TusMarketplaceService {
  ...
  async onboard(
    ...
    if (!esCohorteMercado(cohort) || !this.allowedCohorts.includes(cohort))
      throw new MarketplaceError(400, 'COHORT_NOT_SUPPORTED', 'merchant cohort is outside Stage 1')
    const required = [
      input.merchantId,
      input.locationId,
      input.timezone,
      input.operatingPolicyVersion,
    ]
    if (
      required.some((value) => typeof value !== 'string' || !value.trim()) ||
      !Array.isArray(input.staffRoles) ||
      input.staffRoles.length === 0
    ) {
      throw new MarketplaceError(
        400,
        'INCOMPLETE_MERCHANT',
        'location, timezone, staff roles, and operating policy are required'
      )
    }
    ...
    return this.store.transaction(async (store) => {
      ...
      const audit = crearAuditoriaMercadoServicios(
        {
          tenantId: context.tenantId,
      ...
      await store.outbox.append(
        createOutbox(
          ...
          MARKETPLACE_OUTBOX_EVENT_TYPES.MERCHANT_ONBOARDED,
          'merchant',
          profile.merchantId,
          [audit.auditId]
        )
  ...
  async createListing(
    ...
    if (!merchant || merchant.status !== 'approved')
      throw new MarketplaceError(409, 'MERCHANT_NOT_READY', 'merchant onboarding is incomplete')
    if (input.merchantId !== merchant.merchantId)
      throw new MarketplaceError(
        403,
        'FORBIDDEN',
        'listing merchant is outside the authenticated tenant'
      )
    ...
    const listing: Publicacion = {
      ...
      durationMinutes:
        input.kind === 'service' && bookingMode !== undefined && isFixedBookingMode(bookingMode)
      ...
      ...(input.kind === 'service'
        ? {
            estimatedDurationMinutes:
              bookingMode !== undefined && isEstimatedBookingMode(bookingMode)
                ? input.estimatedDurationMinutes!
                : null,
            priceMode: input.priceMode ?? MARKETPLACE_PRICE_MODES.FIXED,
          }
        : {}),
      createdAt: now,
      updatedAt: now,
    ...
    return this.store.transaction(async (store) => {
      ...
      await store.outbox.append(
        createOutbox(
          ...
          MARKETPLACE_OUTBOX_EVENT_TYPES.LISTING_CREATED,
          'listing',
          listing.listingId,
          [audit.auditId]
        )
  ...
  async publishListing(
    ...
    if (!listing || listing.tenantId !== context.tenantId)
      throw new MarketplaceError(403, 'FORBIDDEN', 'listing is outside the authenticated tenant')
    const merchant = await this.store.merchant.find(context.tenantId)
    if (
      !merchant ||
      merchant.status !== 'approved' ||
      merchant.locationId !== listing.locationId ||
      merchant.cohort !== listing.cohort
    )
      throw new MarketplaceError(
        409,
        'PUBLICATION_INELIGIBLE',
        'merchant and listing facts are not publication eligible'
      )
    ...
    return this.store.transaction(async (store) => {
      ...
      await store.outbox.append(
        createOutbox(
          ...
          MARKETPLACE_OUTBOX_EVENT_TYPES.LISTING_PUBLISHED,
          'listing',
          listingId,
          [audit.auditId]
        )
  ...
  async discover(
    filters: { locationId?: string; cohort?: Cohorte } = {}
  ): Promise<{
    contractVersion: typeof TUS_CONTRACT_VERSION
    ...
    const items = await Promise.all(
      listings.map(async (listing) => {
        ...
        if (
          !merchant ||
          merchant.status !== MARKETPLACE_MERCHANT_STATUSES.APPROVED ||
          merchant.cohort !== listing.cohort ||
          merchant.operatingPolicyVersion !== listing.policyVersion
        )
          return null
        if (merchant.merchantId !== listing.merchantId) return null
        if (listing.kind === 'product' && (listing.stock ?? 0) <= 0) return null
        if (filters.locationId !== undefined && filters.locationId !== listing.locationId)
          return null
        if (filters.cohort !== undefined && filters.cohort !== listing.cohort) return null
        if (listing.kind === 'service') validatePublishedService(listing)
        const calendar =
          listing.kind === 'service'
            ? await this.calendarResolver?.findPrimaryCalendar(listing.tenantId, listing.merchantId)
            : null
        const availabilityStatus: MarketplaceAvailabilityStatus =
          calendar?.status === 'active' ? 'configured' : 'not_configured'
        return {
          contractVersion: TUS_CONTRACT_VERSION,
          ...
          ...(listing.kind === 'product'
            ? { availableQuantity: Math.max(0, listing.stock ?? 0) }
            : {}),
          ...(listing.kind === 'service'
            ? {
                ...
                ...(listing.estimatedDurationMinutes === null ||
                listing.estimatedDurationMinutes === undefined
                  ? {}
                  : { estimatedDurationMinutes: listing.estimatedDurationMinutes }),
                priceMode: listing.priceMode ?? MARKETPLACE_PRICE_MODES.FIXED,
                availabilityStatus,
                ...(availabilityStatus === 'configured' && calendar
    ...
    return {
      contractVersion: TUS_CONTRACT_VERSION,
      items: items.filter((item): item is ItemDescubrimiento => item !== null),
      evidence: 'local-deterministic',
    }
  }
  ...
  async findPublishedService(listingId: string): Promise<Publicacion | null> {
    const listing = await this.store.listings.find(listingId)
    return listing && listing.published && listing.kind === MARKETPLACE_LISTING_KINDS.SERVICE
      ? listing
      : null
  }

  async merchantOperations(
    context: TusAuthenticatedTenantContext
  ): Promise<{ merchant: PerfilPrestador | null; listings: Publicacion[] }> {
    ...
    return {
      merchant,
      listings: merchant && hasLocationAccess(context, merchant.locationId) ? listings : [],
    }
  }

  async checkout(
    input: MarketplaceCheckoutCommand
  ): Promise<{ status: 'executed' | 'replay' } & MarketplaceCheckoutResponse> {
    ...
    if (!input.idempotencyKey.trim() || !input.requestHash.trim())
      throw new MarketplaceError(400, 'INVALID', 'idempotency-key and requestHash are required')
    return this.store.transaction(async (store) => {
        ...
        )
      {
        ...
        for (const line of input.lines) {
          ...
          if (!listing || !listing.published || listing.tenantId.trim().length === 0)
            throw new MarketplaceError(409, 'UNAVAILABLE', 'listing is not currently available')
          const merchant = await store.merchant.find(listing.tenantId)
          if (!merchant || merchant.status !== MARKETPLACE_MERCHANT_STATUSES.APPROVED)
            throw new MarketplaceError(409, 'UNAVAILABLE', 'merchant is not currently available')
          if (merchant.operatingPolicyVersion !== listing.policyVersion)
            throw new MarketplaceError(409, 'STALE_FACTS', 'merchant policy facts are stale', {
              listingId: listing.listingId,
              currentPolicyVersion: merchant.operatingPolicyVersion,
            })
          validateCheckoutLine(line, listing)
          const expectedVersion =
            expectedVersions.get(listing.listingId) ?? listing.availabilityVersion
          if (
            line.availabilityVersion !== expectedVersion ||
            (line.price !== undefined && line.price !== listing.price)
          )
            throw new MarketplaceError(
              409,
              'STALE_FACTS',
              'price or availability facts are stale',
              {
                listingId: listing.listingId,
                currentAvailabilityVersion: listing.availabilityVersion,
                currentPrice: listing.price,
              }
            )
          if (line.context === 'product') {
            const nextQuantity = (productQuantities.get(listing.listingId) ?? 0) + line.quantity
            if (nextQuantity > (listing.stock ?? 0))
              throw new MarketplaceError(409, 'UNAVAILABLE', 'product stock is insufficient', {
                listingId: listing.listingId,
              })
            productQuantities.set(listing.listingId, nextQuantity)
            expectedVersions.set(listing.listingId, listing.availabilityVersion)
          } else if (line.slotStart && line.slotEnd) {
            const bookings = await store.commitments.forListing(listing.listingId)
            const overlapping = bookings.filter(
              (booking) =>
                booking.slotStart &&
                booking.slotEnd &&
                overlaps(line.slotStart!, line.slotEnd!, booking.slotStart, booking.slotEnd)
            ).length
            const overlappingInCheckout = commitments.filter(
              (booking) =>
            ...
            if (overlapping + overlappingInCheckout >= (listing.capacity ?? 0))
              throw new MarketplaceError(
                409,
                'SLOT_UNAVAILABLE',
                'service slot capacity is unavailable',
                { listingId: listing.listingId }
              )
          }
          ...
          audits.push(
            crearAuditoriaMercadoServicios(
              input,
              'commitment.created',
              'commitment',
        ...
        for (const [listingId, quantity] of productQuantities) {
          ...
          if (!listing)
            throw new MarketplaceError(409, 'UNAVAILABLE', 'listing is no longer available', {
              listingId,
            })
          const reserved = await store.listings.reserveProduct({
            tenantId: listing.tenantId,
            listingId,
            availabilityVersion: expectedVersions.get(listingId)!,
            quantity,
          ...
          if (!reserved)
            throw new MarketplaceError(
              409,
              'UNAVAILABLE',
              'product stock or availability changed during checkout',
              { listingId }
            )
        }
        ...
        await store.outbox.append(
          createOutbox(
            input,
            MARKETPLACE_OUTBOX_EVENT_TYPES.COMMITMENTS_CREATED,
            'commitment',
            input.cartId,
            audits.map((audit) => audit.auditId),
            commitments.map((commitment) => commitment.commitmentId)
          )
        )
  ...
  async customerCommitments(
    context: TusAuthenticatedTenantContext
  ): Promise<{ commitments: MarketplaceCommitment[] }> {
    assertPermission(context, 'tus:marketplace:read')
    return { commitments: await this.store.commitments.forTenant(context.tenantId) }
  }

  async customerCommitment(
    context: TusAuthenticatedTenantContext,
    commitmentId: string
  ): Promise<MarketplaceCommitment> {
    assertPermission(context, 'tus:marketplace:read')
    const commitment = await this.store.commitments.find(commitmentId)
    if (!commitment || commitment.tenantId !== context.tenantId)
      throw new MarketplaceError(
        403,
        'FORBIDDEN',
        'marketplace commitment is outside the authenticated tenant'
      )
    return commitment
  }
  ...
  async recordDenied(
    ...
    await this.registrarAuditoriaMercadoServicios(
      context,
      action,
      'authorization',
      resourceId,
  ...
  private async registrarAuditoriaMercadoServicios(
    context: TusAuthenticatedTenantContext,
    action: string,
    resourceType: RegistroAuditoriaMercadoServicios['resourceType'],
    resourceId: string,
    outcome: RegistroAuditoriaMercadoServicios['outcome']
  ): Promise<void> {
  ...
  private requerirHabilitacion(
    context: TusAuthenticatedTenantContext,
    capability: 'publication' | 'settlement'
  ): Promise<unknown> {
    return (
      this.evaluadorHabilitacion?.require({
        tenantId: context.tenantId,
        actorId: context.subjectId,
        correlationId: context.correlationId,
        capability,
...
function crearAuditoriaMercadoServicios(
  input: Pick<MarketplaceCheckoutCommand, 'tenantId' | 'actorId' | 'correlationId' | 'createdAt'>,
  action: string,
  resourceType: RegistroAuditoriaMercadoServicios['resourceType'],
  resourceId: string,
  outcome: RegistroAuditoriaMercadoServicios['outcome']
): RegistroAuditoriaMercadoServicios {
  return {
    auditId: randomUUID(),
    tenantId: input.tenantId,
    actorId: input.actorId,
...
function createOutbox(
  input: Pick<MarketplaceCheckoutCommand, 'tenantId' | 'actorId' | 'correlationId' | 'createdAt'>,
  eventType: MarketplaceOutboxEventType,
  aggregateType: MarketplaceOutboxRecord['aggregateType'],
  aggregateId: string,
  auditIds: string[],
  commitmentIds?: string[]
): MarketplaceOutboxRecord {
  return {
    ...
    payload: {
      correlationId: input.correlationId,
      auditIds,
      ...(commitmentIds === undefined ? {} : { commitmentIds }),
    },
...
function assertPermission(context: TusAuthenticatedTenantContext, permission: string): void {
  if (!context.permissions.includes(permission) && !context.permissions.includes('tus:*'))
    throw new MarketplaceError(403, 'FORBIDDEN', 'TUS marketplace operation is not authorized')
}

function assertMerchantRole(context: TusAuthenticatedTenantContext): void {
  if (
    !context.roles.some((role) =>
      ['merchant', 'merchant-admin', 'owner', 'admin', 'operator'].includes(role)
    )
  )
    throw new MarketplaceError(403, 'FORBIDDEN', 'merchant or operator role is required')
}
...
function validateListingInput(input: EntradaPublicacion, merchant: PerfilPrestador): void {
  if (
    ...
    input.cohort !== merchant.cohort ||
    !/^[A-Z]{3}$/u.test(input.currency.trim().toUpperCase()) ||
    ...
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'listing commercial and location facts are invalid'
    )
  normalizeMoney(input.currency, input.price, input.priceMinor)
  if (input.kind === 'product' && (!Number.isInteger(input.stock) || input.stock! < 0))
    throw new MarketplaceError(400, 'INVALID_LISTING', 'product stock is required')
  if (input.kind === 'service') {
    ...
    if (
      ...
      throw new MarketplaceError(
        400,
        'INVALID_LISTING',
        'service capacity and working hours are required'
      )
    if (
      input.priceMode !== undefined &&
      !Object.values(MARKETPLACE_PRICE_MODES).includes(input.priceMode)
    )
      throw new MarketplaceError(400, 'INVALID_LISTING', 'service price mode is invalid')
  }
}

function resolveBookingMode(
  input: Pick<EntradaPublicacion, 'bookingMode' | 'durationMinutes' | 'estimatedDurationMinutes'>
): MarketplaceBookingMode {
  const mode =
    input.bookingMode ??
    (input.durationMinutes === undefined
      ? MARKETPLACE_BOOKING_MODES.VARIABLE_DURATION
      : MARKETPLACE_BOOKING_MODES.FIXED_SHIFT)
  if (!Object.values(MARKETPLACE_BOOKING_MODES).includes(mode))
    throw new MarketplaceError(400, 'INVALID_LISTING', 'service booking mode is invalid')
  if (
    isFixedBookingMode(mode) &&
    (!Number.isInteger(input.durationMinutes) || input.durationMinutes! <= 0)
  )
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'fixed booking services require durationMinutes'
    )
  if (
    isEstimatedBookingMode(mode) &&
    (!Number.isInteger(input.estimatedDurationMinutes) || input.estimatedDurationMinutes! <= 0)
  )
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'estimated duration services require estimatedDurationMinutes'
    )
  return mode
}
...
function validatePublishedService(listing: Publicacion): void {
  ...
  if (!Object.values(MARKETPLACE_BOOKING_MODES).includes(mode))
    throw new MarketplaceError(400, 'INVALID_LISTING', 'service booking mode is invalid')
  if (!listing.capacity || listing.workingHours.length === 0)
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'service capacity and working hours are required'
  ...
  if (
    ...
    (!listing.durationMinutes ||
      (listing.estimatedDurationMinutes !== null && listing.estimatedDurationMinutes !== undefined))
  )
    throw new MarketplaceError(400, 'INVALID_LISTING', 'fixed booking service duration is invalid')
  if (
    isEstimatedBookingMode(mode) &&
    (!listing.estimatedDurationMinutes || listing.durationMinutes !== null)
  )
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'estimated duration service configuration is invalid'
    )
  if (
    isBudgetBookingMode(mode) &&
    (listing.durationMinutes !== null ||
      (listing.estimatedDurationMinutes !== null && listing.estimatedDurationMinutes !== undefined))
  )
    throw new MarketplaceError(
      400,
      'INVALID_LISTING',
      'budget services cannot expose an automatic duration'
    )
  if (
    listing.priceMode !== undefined &&
    !Object.values(MARKETPLACE_PRICE_MODES).includes(listing.priceMode)
  )
    throw new MarketplaceError(400, 'INVALID_LISTING', 'service price mode is invalid')
}
...
function normalizeMoney(
  ...
  if (!/^[A-Z]{3}$/u.test(normalizedCurrency) || !Number.isFinite(price) || price <= 0)
    throw new MarketplaceError(400, 'INVALID_MONEY', 'currency and price are invalid')
  const derivedMinor = BigInt(Math.round(price * 100))
  if (
    priceMinor !== undefined &&
    (typeof priceMinor !== 'bigint' || priceMinor <= 0n || priceMinor !== derivedMinor)
  )
    throw new MarketplaceError(400, 'INVALID_MONEY', 'price and exact minor units do not match')
  if (
    priceMinor === undefined &&
    Math.abs(price - Number(derivedMinor) / 100) > Number.EPSILON * Math.max(1, price)
  )
    throw new MarketplaceError(400, 'INVALID_MONEY', 'price must resolve to exact minor units')
...
function validateCheckoutLine(line: MarketplaceCheckoutLine, listing: Publicacion): void {
  if (line.context !== listing.kind || !Number.isInteger(line.quantity) || line.quantity <= 0)
    throw new MarketplaceError(
      400,
      'INVALID',
      'checkout context and quantity do not match the listing'
    )
  if (line.context === 'service' && publicationRequiresBudget(listing)) {
    if (line.quantity !== 1 || line.slotStart || line.slotEnd)
      throw new MarketplaceError(
        409,
        'BUDGET_REQUIRED',
        'service requires a budget before automatic booking',
        { listingId: listing.listingId }
      )
    return
  }
  ...
  if (
    line.context === 'service' &&
    (duration === null ||
      line.quantity !== 1 ||
      !line.slotStart ||
      !line.slotEnd ||
      !validInterval(line.slotStart, line.slotEnd) ||
      Date.parse(line.slotEnd) - Date.parse(line.slotStart) !== duration * 60 * 1000)
  )
    throw new MarketplaceError(
      400,
      'INVALID',
      'service checkout requires a valid slot matching the service duration'
    )
}

export function effectiveListingDuration(listing: Publicacion): number | null {
  const mode = resolvedListingBookingMode(listing)
  const duration = isEstimatedBookingMode(mode)
...
export function resolvedListingBookingMode(listing: Publicacion): MarketplaceBookingMode {
  const mode =
    ...
    (listing.durationMinutes === null
      ...
      : MARKETPLACE_BOOKING_MODES.FIXED_SHIFT)
  if (!Object.values(MARKETPLACE_BOOKING_MODES).includes(mode))
    throw new MarketplaceError(409, 'INCONSISTENT_CONFIGURATION', 'service booking mode is invalid')
  return mode
}

export function publicationRequiresBudget(listing: Publicacion): boolean {
  return (
    isBudgetBookingMode(resolvedListingBookingMode(listing)) ||
...
function isFixedBookingMode(mode: MarketplaceBookingMode): boolean {
  return (
    mode === MARKETPLACE_BOOKING_MODES.FIXED_SHIFT ||
    mode === MARKETPLACE_BOOKING_MODES.VISIT_DIAGNOSTIC
  )
}

function isEstimatedBookingMode(mode: MarketplaceBookingMode): boolean {
  return (
    mode === MARKETPLACE_BOOKING_MODES.VARIABLE_DURATION ||
    mode === MARKETPLACE_BOOKING_MODES.ESTIMATED_DURATION
  )
}
...
function hasLocationAccess(context: TusAuthenticatedTenantContext, locationId: string): boolean {
  if (
    context.permissions.includes('tus:*') ||
    context.roles.some((role) => ['owner', 'admin', 'merchant-admin'].includes(role))
  )
    return true
  return (
    context.roles.includes(`location:${locationId}`) ||
    context.permissions.includes(`tus:marketplace:location:${locationId}`)
  )
}
