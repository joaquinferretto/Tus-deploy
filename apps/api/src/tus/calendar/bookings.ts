...
export interface ServiceCalendarStorePort {
  ...
  audit: {
    append(record: CalendarAuditRecord): Promise<void>
    list(tenantId: string): CalendarAuditRecord[]
  }
  outbox: {
    append(record: CalendarOutboxRecord): Promise<void>
    list(tenantId: string): CalendarOutboxRecord[]
  }
...
export class InMemoryServiceCalendarStore implements ServiceCalendarStorePort {
  ...
  readonly calendars = {
    save: async (calendar: Calendario) => {
      this.calendarRecords.set(calendar.calendarId, structuredClone(calendar))
    },
    find: async (calendarId: string) =>
      this.calendarRecords.has(calendarId)
        ? structuredClone(this.calendarRecords.get(calendarId)!)
        : null,
    findPrimary: async (tenantId: string, prestadorId: string) => {
      const matches = [...this.calendarRecords.values()].filter(
        (calendar) => calendar.tenantId === tenantId && calendar.prestadorId === prestadorId
      )
  ...
  readonly bookings = {
    save: async (booking: Reserva) => {
      this.bookingRecords.set(booking.bookingId, structuredClone(booking))
    },
    find: async (bookingId: string) =>
      this.bookingRecords.has(bookingId)
        ? structuredClone(this.bookingRecords.get(bookingId)!)
        : null,
  ...
  readonly idempotency = {
    claim: async ({
      ...
      if (!current) {
        this.idempotencyRecords.set(id, { requestHash })
        return { status: 'claimed' as const }
      }
      if (current.requestHash !== requestHash) return { status: 'conflict' as const }
      return current.response
        ? { status: 'replay' as const, response: structuredClone(current.response) }
    ...
    complete: async ({
      ...
      if (!current)
        throw new ErrorCalendario(500, 'IDEMPOTENCY_MISSING', 'idempotency claim is missing')
  ...
  readonly outbox = {
    append: async (record: CalendarOutboxRecord) => {
      if (!this.outboxRecords.some((item) => item.eventId === record.eventId))
        this.outboxRecords.push(structuredClone(record))
    },
    list: (tenantId: string) =>
      this.outboxRecords
        .filter((record) => record.tenantId === tenantId)
  ...
  private snapshot() {
    return {
      calendars: new Map(
        [...this.calendarRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
      bookings: new Map(
        [...this.bookingRecords].map(([key, value]) => [key, structuredClone(value)])
      ),
      idempotency: new Map(
  ...
  private restore(snapshot: ReturnType<InMemoryServiceCalendarStore['snapshot']>): void {
    ...
    this.auditRecords.push(...snapshot.audits)
    this.outboxRecords.push(...snapshot.outbox)
  }
}
...
export class ServiceCalendarService {
  ...
  async createCalendar(
    ...
    if (input.tenantId !== undefined && input.tenantId !== context.tenantId)
      throw new ErrorCalendario(
        403,
        'FORBIDDEN',
        'calendar tenant does not match authenticated session'
      )
    try {
      ...
      throw new ErrorCalendario(
        400,
        'INVALID_CALENDAR',
        error instanceof Error ? error.message : 'calendar is invalid'
      )
    }
    ...
    const calendar: Calendario = {
      ...
      workingHours: input.workingHours.map((rule) => ({ ...rule })),
      blackoutDates: [...(input.blackoutDates ?? [])],
      policyVersion: input.policyVersion ?? 'calendar-policy-1',
      version: 1,
      createdAt: now,
      updatedAt: now,
    }
  ...
  async findBookingForProvider(tenantId: string, bookingId: string): Promise<Reserva | null> {
    const booking = await this.store.bookings.find(bookingId)
    return booking?.ownerTenantId === tenantId ? booking : null
  }

  async slotsForPublication(
    context: TusAuthenticatedTenantContext,
    ...
    if (publicationRequiresBudget(publication))
      throw new ErrorCalendario(409, 'BUDGET_REQUIRED', 'service requires a budget before booking')
    const durationMinutes = effectiveListingDuration(publication)
    if (durationMinutes === null)
      throw new ErrorCalendario(
        409,
        'INCONSISTENT_CONFIGURATION',
        'service duration configuration is invalid'
      )
    const slots = generateSlotsOrThrow(calendar, date, now, {
  ...
  private async bookInternal(
    ...
    if (input.customerId !== context.subjectId)
      throw new ErrorCalendario(403, 'FORBIDDEN', 'customer does not match authenticated session')
    if (!input.idempotencyKey.trim() || !input.requestHash.trim())
      throw new ErrorCalendario(400, 'INVALID', 'idempotencyKey and requestHash are required')
    if (!input.publication && (!input.calendarId?.trim() || !input.serviceId?.trim()))
      throw new ErrorCalendario(400, 'INVALID', 'calendarId and serviceId are required')
    if (input.publication && (!input.publication.published || input.publication.kind !== 'service'))
    ...
    return this.store.transaction(async (store) => {
      ...
      if (claim.status === 'replay') {
        ...
        const booking =
          response && 'bookingId' in response
            ? response
            : response && 'booking' in response
              ? response.booking
              : null
        if (!booking)
          throw new ErrorCalendario(500, 'IDEMPOTENCY_CORRUPT', 'replay response is invalid')
      ...
      const calendar = input.publication
        ? await store.calendars.findPrimary(
            input.publication.tenantId,
            input.publication.merchantId
          )
      ...
      if (!calendar || calendar.status !== 'active') {
        if (input.publication)
          throw new ErrorCalendario(
            409,
            'NOT_CONFIGURED',
            'service publication has no active provider calendar'
          )
      ...
      if (
        ...
        throw new ErrorCalendario(
          409,
          'CALENDAR_MISMATCH',
          'calendar does not belong to the service publication'
        )
      if (!input.publication && calendar.serviceId !== input.serviceId)
        throw new ErrorCalendario(404, 'NOT_FOUND', 'calendar was not found')
      if (input.publication && publicationRequiresBudget(input.publication))
        throw new ErrorCalendario(
          409,
          'BUDGET_REQUIRED',
          'service requires a budget before booking'
        )
      const durationMinutes = input.publication
        ? effectiveListingDuration(input.publication)
      ...
      const slotOptions: SlotGenerationOptions = {
        durationMinutes,
        ...(input.publication ? { listingId: input.publication.listingId } : {}),
      }
      const slot = generateSlotsOrThrow(calendar, slotDate, undefined, slotOptions).find(
        (candidate) => candidate.slotId === input.slotId
      )
      if (!slot)
        throw new ErrorCalendario(409, 'STALE_SLOT', 'requested slot is no longer available')
      ...
      const booking: Reserva = {
        ...
        bookingId: `booking-${context.tenantId}-${input.idempotencyKey}`,
        tenantId: context.tenantId,
        ownerTenantId: calendar.tenantId,
        ...
        ...(input.publication
          ? {
              ...
              priceSnapshot: structuredClone(input.publication.priceSnapshot),
              policyVersion: input.publication.policyVersion,
            }
          : {
              ...(calendar.serviceId === undefined ? {} : { serviceId: calendar.serviceId }),
              ...(calendar.priceSnapshot === undefined
                ? {}
                : { priceSnapshot: structuredClone(calendar.priceSnapshot) }),
  ...
  private async calendarForPublication(
    publication: Publicacion,
    calendarId?: string
  ): Promise<Calendario> {
    if (!publication.published || publication.kind !== 'service')
      throw new ErrorCalendario(404, 'NOT_FOUND', 'service publication was not found')
    const calendar = await this.store.calendars.findPrimary(
      publication.tenantId,
      publication.merchantId
    )
    if (!calendar || calendar.status !== 'active')
      throw new ErrorCalendario(
        409,
    ...
    if (calendarId !== undefined && calendarId !== calendar.calendarId)
      throw new ErrorCalendario(
        409,
        'CALENDAR_MISMATCH',
        'calendar does not belong to the service publication'
  ...
  async cancel(
    context: TusAuthenticatedTenantContext,
    input: { bookingId: string; now: string; reason: string; expectedVersion?: number }
  ): Promise<Reserva> {
    assertRead(context)
    return this.store.transaction(async (store) => {
      ...
      if (
        !booking ||
        (booking.tenantId !== context.tenantId && booking.ownerTenantId !== context.tenantId)
      )
        throw new ErrorCalendario(403, 'FORBIDDEN', 'booking is outside the authenticated scope')
      if (
        booking.tenantId === context.tenantId &&
        booking.customerId !== context.subjectId &&
        !isOperator(context)
      )
        throw new ErrorCalendario(
          403,
      ...
      if (!calendar)
        throw new ErrorCalendario(409, 'STALE_BOOKING', 'booking calendar is unavailable')
      const late =
        Date.parse(input.now) >
        Date.parse(booking.startsAt) - calendar.cancellationWindowMinutes * 60_000
      const updated = {
        ...booking,
        status: late ? BOOKING_STATUS.CANCELLED_LATE : BOOKING_STATUS.CANCELLED,
      ...
      await this.record(
        store,
        context,
        late ? 'booking.cancelled_late' : 'booking.cancelled',
        booking.bookingId,
        input.now,
        'booking.cancelled',
        booking.bookingId
  ...
  async markNoShow(
    context: TusAuthenticatedTenantContext,
    input: { bookingId: string; now: string }
  ): Promise<Reserva> {
    assertOperator(context)
    return this.store.transaction(async (store) => {
      const booking = await store.bookings.find(input.bookingId)
      ...
      if (
        !calendar ||
        Date.parse(input.now) < Date.parse(booking.endsAt) + calendar.noShowAfterMinutes * 60_000
      )
        throw new ErrorCalendario(
          409,
          'NO_SHOW_TOO_EARLY',
          'no-show evidence window has not opened'
        )
      if (booking.status !== BOOKING_STATUS.CONFIRMED) return booking
      const updated = {
        ...booking,
  ...
  private async record(
    ...
    await store.outbox.append({
      eventId: `calendar-outbox-${audit.auditId}`,
      tenantId: context.tenantId,
      eventType,
      aggregateId: resourceId,
      payload: { auditId: audit.auditId, ...(bookingId === undefined ? {} : { bookingId }) },
      createdAt,
    })
  }
}
...
function assertRead(context: TusAuthenticatedTenantContext): void {
  if (
    !hasPermission(context, 'tus:marketplace:read') &&
    !hasPermission(context, 'tus:calendar:read')
  )
    throw new ErrorCalendario(403, 'FORBIDDEN', 'calendar read is not authorized')
}
function assertCustomer(context: TusAuthenticatedTenantContext): void {
  assertRead(context)
  if (!context.roles.includes('customer'))
    throw new ErrorCalendario(403, 'FORBIDDEN', 'customer role is required')
}
function assertOperator(context: TusAuthenticatedTenantContext): void {
  if (!isOperator(context))
    throw new ErrorCalendario(403, 'FORBIDDEN', 'merchant or operator role is required')
}
function isOperator(context: TusAuthenticatedTenantContext): boolean {
  return (
    context.roles.some((role) => ['merchant', 'operator', 'owner', 'admin'].includes(role)) &&
...
function generateSlotsOrThrow(
  ...
  try {
    return generateServiceSlots(calendar, { date, ...(now === undefined ? {} : { now }) }, options)
  } catch (error) {
    throw new ErrorCalendario(
      400,
      'INVALID_DATE',
      error instanceof Error ? error.message : 'slot date is invalid'
    )
  }
}
  ...
  const active = bookings.filter(
    (booking) =>
      ...
      overlaps(
        ...
        booking.startsAt,
        addMinutes(booking.endsAt, calendar.bufferMinutes)
      )
  ).length
