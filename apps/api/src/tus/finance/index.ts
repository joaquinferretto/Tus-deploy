...
export type ClaveRequisitoFinanciero = (typeof FINANCIAL_GATE_KEYS)[number]
export type ClaveRequisitoDesembolsoFinanciero = (typeof FINANCIAL_PAYOUT_GATE_KEYS)[number]
export type HabilitacionFinanciera = Record<ClaveRequisitoFinanciero, boolean> & Partial<Record<ClaveRequisitoDesembolsoFinanciero, boolean>>
...
export type EstadoPagoProveedorFinanzas = (typeof FINANCE_PROVIDER_STATUSES)[keyof typeof FINANCE_PROVIDER_STATUSES]
export type EstadoComercialFinanzas = 'held' | 'released' | 'frozen' | 'refunded'
...
export interface PoliticaDistribucionPago {
...
export interface IntencionPago {
  ...
  providerStatus: EstadoPagoProveedorFinanzas
  commercialStatus: EstadoComercialFinanzas
  amount: number
  currency: string
  ...
  splitPolicy: PoliticaDistribucionPago
  releaseAt: number
...
export interface EntradaPagoProveedorFinanzas {
  tenantId: string
  commitmentId: string
  amount: number
  currency: string
...
export interface ResultadoPagoProveedorFinanzas {
  providerReference: string
  status: Extract<EstadoPagoProveedorFinanzas, 'pending' | 'approved' | 'rejected'>
}

export interface ProveedorPagoFinanzas {
  readonly source: 'authorized' | 'deterministic-test-only'
  createPaymentIntent(input: EntradaPagoProveedorFinanzas): Promise<ResultadoPagoProveedorFinanzas>
}

export interface TransporteJobLiberacionFinanzas {
  enqueue(input: { tenantId: string; jobId: string }): Promise<{ status: 'queued'; tenantId: string; jobId: string }>
}

export interface InstantaneaComision {
  contractVersion: typeof TUS_CONTRACT_VERSION
  snapshotId: string
  tenantId: string
  commitmentId: string
...
export interface EvidenciaFinanciera {
  contractVersion: typeof TUS_CONTRACT_VERSION
  evidenceId: string
  tenantId: string
  commitmentId: string
...
export interface CongelamientoFinanciero {
  freezeId: string
  tenantId: string
  commitmentId: string
...
export interface ConfirmacionCumplimiento {
...
interface RegistroIdempotenciaFinanzas {
  requestHash: string
  response: unknown
}

export interface PuertoAlmacenFinanzas {
  getPayment(tenantId: string, commitmentId: string): MaybePromise<IntencionPago | null>
  savePayment(payment: IntencionPago): MaybePromise<IntencionPago>
  getSnapshot(tenantId: string, commitmentId: string): MaybePromise<InstantaneaComision | null>
  saveSnapshot(snapshot: InstantaneaComision): MaybePromise<InstantaneaComision>
  appendLedger(entry: MovimientoContable): MaybePromise<MovimientoContable>
  listLedger(tenantId: string, commitmentId: string): MaybePromise<MovimientoContable[]>
  saveEvidence(evidence: EvidenciaFinanciera): MaybePromise<EvidenciaFinanciera>
  listEvidence(tenantId: string, commitmentId: string): MaybePromise<EvidenciaFinanciera[]>
  saveConfirmation(confirmation: ConfirmacionCumplimiento): MaybePromise<ConfirmacionCumplimiento>
  getConfirmation(tenantId: string, commitmentId: string): MaybePromise<ConfirmacionCumplimiento | null>
  saveFreeze(freeze: CongelamientoFinanciero): MaybePromise<CongelamientoFinanciero>
  getFreeze(tenantId: string, commitmentId: string): MaybePromise<CongelamientoFinanciero | null>
  getIdempotency(tenantId: string, key: string): MaybePromise<RegistroIdempotenciaFinanzas | null>
  saveIdempotency(tenantId: string, key: string, record: RegistroIdempotenciaFinanzas): MaybePromise<void>
  getReconciliation(tenantId: string, commitmentId: string): MaybePromise<ResultadoConciliacion | null>
  saveReconciliation(result: ResultadoConciliacion): MaybePromise<ResultadoConciliacion>
}
...
export interface EntradaWebhookFinanzas {
  tenantId: string
  actorId: string
  correlationId: string
  commitmentId: string
  paymentId: string
  providerReference: string
  status: EstadoPagoProveedorFinanzas
  amount: number
  currency: string
  eventId: string
  requestId: string
...
export type ResultadoWebhookFinanzas =
  | { status: 'processed'; payment: IntencionPago }
  | { status: 'replay'; payment: IntencionPago | null }
  | { status: 'out_of_order'; payment: IntencionPago }
  | { status: 'rejected'; reason: 'invalid_signature' | 'expired_signature' | 'invalid_request'; payment: IntencionPago | null }
...
export class InMemoryFinanceStore implements PuertoAlmacenFinanzas {
  private readonly payments = new Map<string, IntencionPago>()
  private readonly snapshots = new Map<string, InstantaneaComision>()
  private readonly ledger = new Map<string, MovimientoContable>()
  private readonly evidence = new Map<string, EvidenciaFinanciera>()
  private readonly confirmations = new Map<string, ConfirmacionCumplimiento>()
  private readonly freezes = new Map<string, CongelamientoFinanciero>()
  private readonly idempotency = new Map<string, RegistroIdempotenciaFinanzas>()
  private readonly reconciliations = new Map<string, ResultadoConciliacion>()

  getPayment(tenantId: string, commitmentId: string): IntencionPago | null {
    return clone(this.payments.get(key(tenantId, commitmentId)) ?? null)
  }

  savePayment(payment: IntencionPago): IntencionPago {
    this.payments.set(key(payment.tenantId, payment.commitmentId), clone(payment))
    return clone(payment)
  }

  getSnapshot(tenantId: string, commitmentId: string): InstantaneaComision | null {
    const snapshot = this.snapshots.get(key(tenantId, commitmentId))
    return snapshot ? Object.freeze(clone(snapshot)) : null
  }

  saveSnapshot(snapshot: InstantaneaComision): InstantaneaComision {
  ...
  appendLedger(entry: MovimientoContable): MovimientoContable {
    if (!esImporteEnUnidadesMenoresSeguro(entry.amount) || !entry.currency.trim()) throw new FinanceError(400, 'INVALID_MONEY', 'ledger amount must be an exact minor unit')
  ...
  listLedger(tenantId: string, commitmentId: string): MovimientoContable[] {
    return [...this.ledger.values()]
      .filter((entry) => entry.tenantId === tenantId && entry.commitmentId === commitmentId)
      .map(clone)
  }

  saveEvidence(evidence: EvidenciaFinanciera): EvidenciaFinanciera {
    this.evidence.set(key(evidence.tenantId, evidence.evidenceId), clone(evidence))
    return clone(evidence)
  }

  listEvidence(tenantId: string, commitmentId: string): EvidenciaFinanciera[] {
    return [...this.evidence.values()]
      .filter((evidence) => evidence.tenantId === tenantId && evidence.commitmentId === commitmentId)
      .map(clone)
  }

  saveConfirmation(confirmation: ConfirmacionCumplimiento): ConfirmacionCumplimiento {
    this.confirmations.set(key(confirmation.tenantId, confirmation.commitmentId), clone(confirmation))
    return clone(confirmation)
  }

  getConfirmation(tenantId: string, commitmentId: string): ConfirmacionCumplimiento | null {
    return clone(this.confirmations.get(key(tenantId, commitmentId)) ?? null)
  }

  saveFreeze(freeze: CongelamientoFinanciero): CongelamientoFinanciero {
  ...
  getFreeze(tenantId: string, commitmentId: string): CongelamientoFinanciero | null {
    return clone(this.freezes.get(key(tenantId, commitmentId)) ?? null)
  }

  getIdempotency(tenantId: string, idempotencyKey: string): RegistroIdempotenciaFinanzas | null {
    return clone(this.idempotency.get(key(tenantId, idempotencyKey)) ?? null)
  }

  saveIdempotency(tenantId: string, idempotencyKey: string, record: RegistroIdempotenciaFinanzas): void {
    this.idempotency.set(key(tenantId, idempotencyKey), clone(record))
  }

  getReconciliation(tenantId: string, commitmentId: string): ResultadoConciliacion | null {
    return clone(this.reconciliations.get(key(tenantId, commitmentId)) ?? null)
  }

  saveReconciliation(result: ResultadoConciliacion): ResultadoConciliacion {
    this.reconciliations.set(key(result.tenantId, result.commitmentId), clone(result))
    return clone(result)
  }
}

export class DeterministicMercadoPagoFinanceProvider implements ProveedorPagoFinanzas {
  readonly source = 'deterministic-test-only' as const
  createCalls = 0
  private next: ResultadoPagoProveedorFinanzas = { providerReference: 'deterministic-payment', status: 'pending' }

  setNext(result: ResultadoPagoProveedorFinanzas): void {
    this.next = { ...result }
  }

  async createPaymentIntent(_input: EntradaPagoProveedorFinanzas): Promise<ResultadoPagoProveedorFinanzas> {
    this.createCalls += 1
    return { ...this.next }
  }
}

export class UnavailableMercadoPagoFinanceProvider implements ProveedorPagoFinanzas {
...
export interface OpcionesServicioFinanzas {
  ...
  ruleVersion?: string
  releaseJobs?: TransporteJobLiberacionFinanzas
  ...
  splitPolicy?: Partial<PoliticaDistribucionPago>
  enforceFiveDayHold?: boolean
}

export class TusFinanceService {
  readonly store: PuertoAlmacenFinanzas
  private readonly provider: ProveedorPagoFinanzas
  private readonly readiness: HabilitacionFinanciera
  private readonly commitmentLookup: OpcionesServicioFinanzas['commitmentLookup']
  private readonly now: () => number
  private readonly commissionRateBps: number
  private readonly ruleVersion: string
  private readonly releaseJobs?: TransporteJobLiberacionFinanzas
  ...
  private readonly splitPolicy: PoliticaDistribucionPago
  private readonly enforceFiveDayHold: boolean

  constructor(options: OpcionesServicioFinanzas) {
  ...
  readinessStatus(): { enabled: boolean; failedGates: ClaveRequisitoFinanciero[] } {
  ...
  payoutReadinessStatus(): { enabled: boolean; failedGates: ClaveRequisitoDesembolsoFinanciero[] } {
    const failedGates = FINANCIAL_PAYOUT_GATE_KEYS.filter((gate) => this.readiness[gate] !== true)
    return { enabled: failedGates.length === 0, failedGates }
  }

  async enqueueReleaseJob(input: ContextoComandoFinanzas & { commitmentId: string }): Promise<{ status: 'queued'; tenantId: string; jobId: string }> {
  ...
  async createPaymentIntent(input: ContextoComandoFinanzas & { commitmentId: string; orderId?: string; posOperationId?: string | null; idempotencyKey: string; requestHash: string }): Promise<ResultadoIntencionPago> {
    validarContextoFinanzas(input)
    ...
    if (existing) {
      ...
      return { status: 'replay', ...(clone(existing.response) as { payment: IntencionPago }) }
    }
    const commitment = await this.requerirCompromiso(input.tenantId, input.commitmentId)
    const now = this.now()
    const gates = this.readinessStatus()
    let payment: IntencionPago
    if (!gates.enabled || !this.providerEnabled) {
      payment = crearIntencionPago(input, commitment, null, 'pending', 'held', 'held-no-provider', now, this.splitPolicy)
    ...
    let providerPayment: ResultadoPagoProveedorFinanzas
    try {
      providerPayment = await this.crearPagoProveedor({
      ...
      payment = crearIntencionPago(input, commitment, null, 'provider_error', 'frozen', 'held-no-provider', now, this.splitPolicy)
    ...
    payment = crearIntencionPago(input, commitment, providerPayment.providerReference, providerPayment.status, 'held', this.provider.source, now, this.splitPolicy)
    await this.store.savePayment(payment)
    if (providerPayment.status === 'approved') await this.registrarInstantaneaComisionAutoritativa(input, commitment, payment, now)
    const response = { status: 'created' as const, payment }
    await this.store.saveIdempotency(input.tenantId, input.idempotencyKey, { requestHash: input.requestHash, response })
    return response
  }

  async handleWebhook(input: EntradaWebhookFinanzas, nowMs = this.now()): Promise<ResultadoWebhookFinanzas> {
    validarContextoFinanzas(input)
    if (!input.eventId.trim() || !input.requestId.trim() || !input.paymentId.trim() || !input.providerReference.trim() || !esImporteEnUnidadesMenoresSeguro(input.amount) || !input.currency.trim() || !Number.isSafeInteger(input.timestamp) || input.timestamp <= 0) {
    ...
    const payment = await this.requerirIntencionPago(input.tenantId, input.commitmentId)
    ...
    if (existing) return { status: 'replay', payment: clone(existing.response) as IntencionPago | null }
    const eventAt = input.timestamp * 1_000
    if (payment.providerEventAt !== null && eventAt <= payment.providerEventAt) {
      await this.store.saveIdempotency(input.tenantId, idempotencyKey, { requestHash: hashWebhookFinanzas(input), response: payment })
      return { status: 'out_of_order', payment }
    }
    if (!esTransicionProveedorPagoPermitida(payment.providerStatus, input.status)) return { status: 'rejected', reason: 'invalid_request', payment }
    ...
    if (input.status === 'approved' && !await this.store.getSnapshot(input.tenantId, input.commitmentId)) {
      await this.registrarInstantaneaComisionAutoritativa({ tenantId: input.tenantId, actorId: input.actorId, correlationId: input.correlationId, commitmentId: input.commitmentId }, await this.requerirCompromiso(input.tenantId, input.commitmentId), updated, this.now())
    }
    if (input.status === 'charged_back') await this.freeze({ ...input, reason: 'chargeback' })
    await this.store.saveIdempotency(input.tenantId, idempotencyKey, { requestHash: hashWebhookFinanzas(input), response: updated })
    return { status: 'processed', payment: updated }
  }

  private async crearPagoProveedor(input: EntradaPagoProveedorFinanzas): Promise<ResultadoPagoProveedorFinanzas> {
    ...
    const reason = motivoErrorProveedorPago(lastError)
  ...
  async confirmCompletion(input: ContextoComandoFinanzas & { commitmentId: string; confirmationId: string; confirmedAt: string }): Promise<ResultadoLiberacion> {
    ...
    await this.requerirCompromiso(input.tenantId, input.commitmentId)
  ...
  async release(input: ContextoComandoFinanzas & { commitmentId: string; now: string }): Promise<ResultadoLiberacion> {
    validarContextoFinanzas(input)
    await this.requerirHabilitacion(input, 'settlement')
    ...
    if (payment.commercialStatus === 'released') {
      ...
      return { status: 'released', reason: motivoLiberacion(previousRelease?.reason) }
    }
    ...
    await this.store.appendLedger(this.crearMovimientoContable(input, 'merchant_release', payment.amount - ((await this.store.getSnapshot(input.tenantId, input.commitmentId))?.commissionAmount ?? 0), payment.currency, eligibility.reason, null))
    return { status: 'released', reason: eligibility.reason }
  }

  async freeze(input: ContextoComandoFinanzas & { commitmentId: string; reason: CongelamientoFinanciero['reason'] }): Promise<{ status: 'frozen'; freeze: CongelamientoFinanciero }> {
    validarContextoFinanzas(input)
    await this.requerirHabilitacion(input, 'settlement')
    await this.requerirIntencionPago(input.tenantId, input.commitmentId)
  ...
  async openDispute(input: ContextoComandoFinanzas & { commitmentId: string }): Promise<{ status: 'frozen'; freeze: CongelamientoFinanciero }> {
    return this.freeze({ ...input, reason: 'dispute' })
  }

  async refund(input: ContextoComandoFinanzas & { commitmentId: string; amount: number; reason: string; idempotencyKey: string }): Promise<ResultadoReintegro> {
    validarContextoFinanzas(input)
    await this.requerirHabilitacion(input, 'settlement')
    ...
    if (existing) {
      ...
      return clone(existing.response) as ResultadoReintegro
    }
    const payment = await this.requerirIntencionPago(input.tenantId, input.commitmentId)
    if (!esImporteEnUnidadesMenoresSeguro(input.amount) || input.amount <= 0) throw new FinanceError(400, 'INVALID_REFUND', 'refund amount is outside the payment amount')
    const refundedAmount = sumarImportesEnUnidadesMenores((await this.store.listLedger(input.tenantId, input.commitmentId)).filter((entry) => entry.entryType === 'refund_compensation').map((entry) => entry.amount))
    ...
    const result: ResultadoReintegro = { status: 'compensated', refundId: `refund-${input.commitmentId}-${input.idempotencyKey}`, commitmentId: input.commitmentId, amount: input.amount, reason: input.reason, deterministic: this.provider.source === 'deterministic-test-only' }
    await this.freeze({ ...input, reason: 'refund' })
    await this.store.appendLedger(this.crearMovimientoContable(input, 'refund_compensation', input.amount, payment.currency, input.reason, `gross-${input.commitmentId}`, result.refundId))
  ...
  async chargeback(input: ContextoComandoFinanzas & { commitmentId: string }): Promise<{ status: 'frozen'; freeze: CongelamientoFinanciero }> {
    const result = await this.freeze({ ...input, reason: 'chargeback' })
    const payment = await this.requerirIntencionPago(input.tenantId, input.commitmentId)
    await this.store.appendLedger(this.crearMovimientoContable(input, 'chargeback_compensation', payment.amount, payment.currency, 'provider-chargeback', null, `chargeback-${input.commitmentId}`))
    return result
  }

  async reconcile(input: ContextoComandoFinanzas & { commitmentId: string; providerReference: string; providerAmount: number }): Promise<ResultadoConciliacion> {
    validarContextoFinanzas(input)
    await this.requerirHabilitacion(input, 'settlement')
    const existing = await this.store.getReconciliation(input.tenantId, input.commitmentId)
    if (existing) return existing
    const payment = await this.requerirIntencionPago(input.tenantId, input.commitmentId)
    const matches = payment.providerReference === input.providerReference && esImporteEnUnidadesMenoresSeguro(input.providerAmount) && payment.amount === input.providerAmount
    ...
    if (!matches) {
      await this.freeze({ ...input, reason: 'provider-mismatch' })
      await this.store.appendLedger(this.crearMovimientoContable(input, 'reconciliation_compensation', input.providerAmount, payment.currency, `reconciliation:${result.reason}`, `gross-${input.commitmentId}`, `reconciliation-${input.commitmentId}`))
    }
    return result
  }
  ...
  private async requerirIntencionPago(tenantId: string, commitmentId: string): Promise<IntencionPago> {
  ...
  private async registrarInstantaneaComisionAutoritativa(input: ContextoComandoFinanzas & { commitmentId: string }, commitment: TusCommitment, payment: IntencionPago, now: number): Promise<InstantaneaComision> {
    ...
    await this.store.appendLedger(this.crearMovimientoContable(ledgerContext, 'gross_authorized', commitment.amount, commitment.currency, 'provider-approved', null, `gross-${commitment.commitmentId}`))
    await this.store.appendLedger(this.crearMovimientoContable(ledgerContext, 'commission_reserved', commissionAmount, commitment.currency, this.ruleVersion, `gross-${commitment.commitmentId}`, `commission-${commitment.commitmentId}`))
    await this.store.appendLedger(this.crearMovimientoContable(ledgerContext, 'merchant_payable_held', commitment.amount - commissionAmount, commitment.currency, 'confirmation-first-hold', `commission-${commitment.commitmentId}`, `payable-${commitment.commitmentId}`))
    return snapshot
  }

  private crearMovimientoContable(input: ContextoComandoFinanzas & { commitmentId: string }, entryType: TipoMovimientoContable, amount: number, currency: string, reason: string, linkedEntryId: string | null, entryId = `${entryType}-${input.commitmentId}-${this.now()}`): MovimientoContable {
...
export type ContextoComandoFinanzas = { tenantId: string; actorId: string; correlationId: string }
export type ResultadoIntencionPago =
  | { status: 'created'; payment: IntencionPago }
  | { status: 'held'; reason: 'financial_gates_incomplete' | 'provider_disabled'; payment: IntencionPago }
...
export function createCommissionSnapshot(input: Omit<InstantaneaComision, 'contractVersion'>): InstantaneaComision {
  if (!esImporteEnUnidadesMenoresSeguro(input.grossAmount) || !esImporteEnUnidadesMenoresSeguro(input.deductions) || !esImporteEnUnidadesMenoresSeguro(input.commissionableBase) || !esImporteEnUnidadesMenoresSeguro(input.commissionAmount) || !esImporteEnUnidadesMenoresSeguro(input.netAmount)) throw new Error('commission amounts must be non-negative exact minor units')
  ...
  if (input.commissionAmount !== calcularImporteComision(input.commissionableBase, input.rateBps)) throw new Error('commission amount does not match the snapshotted rate and base')
...
export function verifyFinanceWebhookSignature(secret: string, input: Pick<EntradaWebhookFinanzas, 'eventId' | 'requestId' | 'timestamp' | 'signature'>): boolean {
...
function esImporteEnUnidadesMenoresSeguro(value: number): boolean {
  return Number.isSafeInteger(value) && value >= 0
}

function sumarImportesEnUnidadesMenores(values: number[]): number {
  const total = values.reduce((sum, value) => sum + value, 0)
  if (!esImporteEnUnidadesMenoresSeguro(total)) throw new FinanceError(409, 'MONEY_OVERFLOW', 'money total exceeds the supported exact range')
  return total
}

function calcularImporteComision(base: number, rateBps: number): number {
  const amount = (BigInt(base) * BigInt(rateBps) + 5_000n) / 10_000n
  const normalized = Number(amount)
  if (!esImporteEnUnidadesMenoresSeguro(normalized)) throw new FinanceError(409, 'MONEY_OVERFLOW', 'commission exceeds the supported exact range')
  return normalized
}

function validarDineroExacto(currency: string, minor: number): void {
  createFinanceMoney(currency, BigInt(minor))
}

function motivoErrorProveedorPago(error: unknown): 'timeout' | 'unavailable' {
  return error instanceof Error && (error as Error & { code?: string }).code === 'PROVIDER_TIMEOUT' ? 'timeout' : 'unavailable'
}

function hashWebhookFinanzas(input: EntradaWebhookFinanzas): string {
  return `${input.eventId}:${input.paymentId}:${input.status}:${input.timestamp}`
}

function esTransicionProveedorPagoPermitida(current: EstadoPagoProveedorFinanzas, next: EstadoPagoProveedorFinanzas): boolean {
...
function validarContextoFinanzas(input: ContextoComandoFinanzas): void {
