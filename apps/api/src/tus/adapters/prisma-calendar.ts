...
export class PrismaServiceCalendarStore implements ServiceCalendarStorePort {
  ...
  readonly calendars = {
    ...
    findPrimary: async (tenantId: string, prestadorId: string) => {
      const row = await this.client.calendario.findUnique({ where: { tenantId_prestadorId: { tenantId, prestadorId } } })
      if (!row) return null
      const rules = await this.client.reglaCalendario.findMany({ where: { calendarioId: String(row['id']), tenantId } })
      const exceptions = await this.client.excepcionCalendario.findMany({ where: { calendarioId: String(row['id']), tenantId, estado: 'active' } })
      return toCalendar(row, rules, exceptions)
    },
  }
...
function bookingRow(booking: Reserva): Record<string, unknown> { return { id: booking.bookingId, tenantId: booking.ownerTenantId, reservaId: booking.bookingId, servicioId: booking.serviceId ?? null, calendarioId: booking.calendarId, publicacionId: booking.listingId ?? null, clienteId: booking.customerId, fechaInicio: new Date(booking.startsAt), fechaFin: new Date(booking.endsAt), estado: booking.status, version: booking.version, fechaCreacion: new Date(booking.createdAt), fechaActualizacion: new Date(booking.updatedAt) } }
function toCalendar(row: Record<string, unknown>, rules: Record<string, unknown>[], exceptions: Record<string, unknown>[]): Calendario {
  ...
  const input: EntradaCalendario = { calendarId: String(row['id']), ...(row['prestadorId'] === null || row['prestadorId'] === undefined ? {} : { prestadorId: String(row['prestadorId']) }), ...(row['servicioId'] === null || row['servicioId'] === undefined ? {} : { serviceId: String(row['servicioId']) }), timezone: String(row['zonaHoraria']), durationMinutes: 60, granularityMinutes: Number(row['granularidadMinutos'] ?? 15), bufferMinutes: Number(row['bufferMinutos'] ?? 0), capacity: ruleCapacity, workingHours: rules.map((rule) => ({ weekday: Number(rule['diaSemana']), start: String(rule['horaInicio']), end: String(rule['horaFin']) })), blackoutDates: exceptions.map((exception) => new Date(String(exception['fechaInicio'])).toISOString().slice(0, 10)) }
  return { ...input, tenantId, status: String(row['estado']) as Calendario['status'], durationMinutes: input.durationMinutes ?? 60, granularityMinutes: input.granularityMinutes ?? 15, bufferMinutes: input.bufferMinutes ?? 0, bookingCutoffMinutes: 0, cancellationWindowMinutes: 0, noShowAfterMinutes: 0, workingHours: input.workingHours, blackoutDates: input.blackoutDates ?? [], policyVersion: 'calendar-policy-1', version: 1, createdAt: new Date(String(row['fechaCreacion'])).toISOString(), updatedAt: new Date(String(row['fechaActualizacion'])).toISOString() }
}
function toBooking(row: Record<string, unknown>): Reserva { return { contractVersion: TUS_CONTRACT_VERSION, bookingId: String(row['reservaId']), tenantId: String(row['tenantId']), ownerTenantId: String(row['tenantId']), ...(row['servicioId'] === null || row['servicioId'] === undefined ? {} : { serviceId: String(row['servicioId']) }), calendarId: String(row['calendarioId']), ...(row['publicacionId'] === null || row['publicacionId'] === undefined ? {} : { listingId: String(row['publicacionId']) }), customerId: String(row['clienteId']), startsAt: new Date(String(row['fechaInicio'])).toISOString(), endsAt: new Date(String(row['fechaFin'])).toISOString(), status: row['estado'] as Reserva['status'], version: Number(row['version']), policyVersion: 'calendar-policy-1', createdAt: new Date(String(row['fechaCreacion'])).toISOString(), updatedAt: new Date(String(row['fechaActualizacion'])).toISOString() } }
