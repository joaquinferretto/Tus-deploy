...
import type { CalendarAuditRecord, CalendarOutboxRecord, ServiceBooking, ServiceCalendarStorePort } from '../calendar/bookings.ts'
import type { ServiceCalendar, ServiceCalendarInput } from '../calendar/rules.ts'
...
export class PrismaServiceCalendarStore implements ServiceCalendarStorePort {
  ...
  readonly calendars = {
    save: async (calendar: ServiceCalendar) => {
  ...
  readonly bookings = {
    save: async (booking: ServiceBooking) => { await this.client.tusBooking.upsert({ where: { id: booking.bookingId }, create: bookingRow(booking), update: bookingRow(booking) }) },
  ...
  readonly idempotency = {
    claim: async ({ tenantId, key, requestHash }: { tenantId: string; key: string; requestHash: string }): Promise<{ status: 'claimed' | 'replay' | 'conflict' | 'in_progress'; response?: ServiceBooking | { status: 'replay'; booking: ServiceBooking } | { status: 'rejected'; reason: 'capacity' } }> => {
    ...
    complete: async ({ tenantId, key, response }: { tenantId: string; key: string; response: ServiceBooking | { status: 'replay'; booking: ServiceBooking } | { status: 'rejected'; reason: 'capacity' } }) => { await this.client.idempotencyRecord.update({ where: { tenantId_key: { tenantId, key } }, data: { status: 'completed', response: encodeResult(response) } }) },
  }
...
function calendarRow(calendar: ServiceCalendar): Record<string, unknown> { return { id: calendar.calendarId, tenantId: calendar.tenantId, serviceId: calendar.serviceId, name: calendar.serviceId, timezone: calendar.timezone, status: calendar.status, createdAt: new Date(calendar.createdAt), updatedAt: new Date(calendar.updatedAt) } }
function bookingRow(booking: ServiceBooking): Record<string, unknown> { return { id: booking.bookingId, tenantId: booking.ownerTenantId, bookingId: booking.bookingId, serviceId: booking.serviceId, calendarId: booking.calendarId, customerId: booking.customerId, startsAt: new Date(booking.startsAt), endsAt: new Date(booking.endsAt), status: booking.status, version: booking.version, createdAt: new Date(booking.createdAt), updatedAt: new Date(booking.updatedAt) } }
function toCalendar(row: Record<string, unknown>, rules: Record<string, unknown>[], exceptions: Record<string, unknown>[]): ServiceCalendar {
  const tenantId = String(row['tenantId'])
  const ruleCapacity = Number(rules[0]?.['capacity'] ?? 1)
  const input: ServiceCalendarInput = { calendarId: String(row['id']), serviceId: String(row['serviceId']), timezone: String(row['timezone']), durationMinutes: 60, capacity: ruleCapacity, workingHours: rules.map((rule) => ({ weekday: Number(rule['weekday']), start: String(rule['startsAt']), end: String(rule['endsAt']) })), blackoutDates: exceptions.map((exception) => new Date(String(exception['startsAt'])).toISOString().slice(0, 10)) }
  return { ...input, tenantId, status: String(row['status']) as ServiceCalendar['status'], bufferMinutes: 0, bookingCutoffMinutes: 0, cancellationWindowMinutes: 0, noShowAfterMinutes: 0, workingHours: input.workingHours, blackoutDates: input.blackoutDates ?? [], policyVersion: 'calendar-policy-1', version: 1, createdAt: new Date(String(row['createdAt'])).toISOString(), updatedAt: new Date(String(row['updatedAt'])).toISOString() }
}
function toBooking(row: Record<string, unknown>): ServiceBooking { return { contractVersion: TUS_CONTRACT_VERSION, bookingId: String(row['bookingId']), tenantId: String(row['tenantId']), ownerTenantId: String(row['tenantId']), serviceId: String(row['serviceId']), calendarId: String(row['calendarId']), customerId: String(row['customerId']), startsAt: new Date(String(row['startsAt'])).toISOString(), endsAt: new Date(String(row['endsAt'])).toISOString(), status: row['status'] as ServiceBooking['status'], version: Number(row['version']), policyVersion: 'calendar-policy-1', createdAt: new Date(String(row['createdAt'])).toISOString(), updatedAt: new Date(String(row['updatedAt'])).toISOString() } }
...
function decodeResult(value: unknown): ServiceBooking | { status: 'replay'; booking: ServiceBooking } | { status: 'rejected'; reason: 'capacity' } { return JSON.parse(JSON.stringify(value), (key, item) => key === 'minor' && typeof item === 'string' && /^\d+n$/u.test(item) ? BigInt(item.slice(0, -1)) : item) as ServiceBooking | { status: 'replay'; booking: ServiceBooking } | { status: 'rejected'; reason: 'capacity' } }
