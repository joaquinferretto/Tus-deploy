...
export class PrismaMarketplaceStore implements MarketplaceStorePort {
  ...
  readonly listings = {
    save: async (listing: Publicacion) => {
      await this.client.publicacion.upsert({
        where: { id: listing.listingId },
        create: listingRow(listing),
        update: listingRow(listing),
      })
    },
    ...
    reserveProduct: async ({
      ...
      const result = await this.client.publicacion.updateMany({
        where: {
          ...
          tipo: 'product',
          publicada: true,
          versionDisponibilidad: availabilityVersion,
          existencias: { gte: quantity },
        },
        data: {
          existencias: { decrement: quantity },
          versionDisponibilidad: { increment: 1 },
          fechaActualizacion: new Date(updatedAt),
        },
      })
  ...
  readonly commitments = {
    saveMany: async (commitments: readonly MarketplaceCommitment[]) => {
      const rows = await Promise.all(
        commitments.map(async (commitment) => {
          const listing = await this.client.publicacion.findUnique({
            where: { id: commitment.listingId },
          })
          if (!listing) throw new Error('marketplace commitment listing was not found')
          return commitmentRow(commitment, String(listing['tenantId']))
        })
      )
    ...
    forTenant: async (tenantId: string) =>
      (await this.client.compromisoMercadoServicios.findMany({ where: { tenantId } })).map(
        toCommitment
      ),
    forListing: async (listingId: string) =>
      (
        await this.client.compromisoMercadoServicios.findMany({
          where: { publicacionId: listingId },
        })
      ).map(toCommitment),
  }
  ...
  readonly audit = {
    ...
    list: async (tenantId: string): Promise<RegistroAuditoriaMercadoServicios[]> =>
      (await this.client.auditoriaMercadoServicios.findMany({ where: { tenantId } })).map(
        mapearAuditoriaMercadoServicios
      ),
  }
  ...
  readonly idempotency = {
      ...
      if (!existing) {
        try {
          await this.client.idempotencyRecord.create({
            data: {
              id: `marketplace-idempotency-${tenantId}-${key}`,
              tenantId,
              key,
              requestHash,
              status: 'pending',
    ...
    complete: async ({
      ...
      await this.client.idempotencyRecord.update({
        where: { tenantId_key: { tenantId, key } },
        data: { status: 'completed', response: encodeMarketplaceResponse(response) },
      })
    },
  ...
  readonly outbox = {
    ...
    list: async (tenantId: string): Promise<MarketplaceOutboxRecord[]> =>
      (
        await this.client.outboxEvent.findMany({
          where: { tenantId, eventType: { startsWith: 'tus.marketplace.' } },
        })
      ).map(toOutbox),
  }
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        return await this.client.$transaction(
          async (client) => operation(new PrismaMarketplaceStore(client)),
...
function claimExistingMarketplaceIdempotency(
  ...
  if (existing.status === 'completed' && existing.response)
    return { status: 'replay' as const, response: decodeMarketplaceResponse(existing.response) }
...
function merchantRow(profile: PerfilPrestador): Record<string, unknown> {
  return {
    ...
    prestadorId: profile.merchantId,
    cohorte: profile.cohort,
    ubicacionId: profile.locationId,
...
function listingRow(listing: Publicacion): Record<string, unknown> {
  return {
    ...
    versionContrato: listing.contractVersion,
    tenantId: listing.tenantId,
    prestadorId: listing.merchantId,
    tipo: listing.kind,
    nombre: listing.name,
    descripcion: listing.description,
    cohorte: listing.cohort,
    ubicacionId: listing.locationId,
    moneda: listing.currency,
    precio: BigInt(listing.priceMinor),
    versionDisponibilidad: listing.availabilityVersion,
    publicada: listing.published,
    versionPolitica: listing.policyVersion,
    existencias: listing.stock,
    duracionMinutos: listing.durationMinutes,
    capacidad: listing.capacity,
    modalidadReserva: toStoredBookingMode(listing.bookingMode),
    duracionEstimadaMinutos: listing.estimatedDurationMinutes ?? null,
    modalidadPrecio: toStoredPriceMode(listing.priceMode),
    horarioTrabajo: listing.workingHours,
    fechaCreacion: new Date(listing.createdAt),
    fechaActualizacion: new Date(listing.updatedAt),
  }
}
...
function commitmentRow(
  ...
  return {
    ...
    cantidad: commitment.quantity,
    monto: BigInt(commitment.priceSnapshot.minor) * BigInt(commitment.quantity),
    moneda: commitment.priceSnapshot.currency,
    estado: commitment.status,
    versionDisponibilidad: commitment.availabilityVersion,
    versionPolitica: commitment.policyVersion,
    franjaInicio: commitment.slotStart ? new Date(commitment.slotStart) : null,
    franjaFin: commitment.slotEnd ? new Date(commitment.slotEnd) : null,
...
function toMerchant(row: Record<string, unknown>): PerfilPrestador {
  return {
    ...
    merchantId: String(row['prestadorId']),
    cohort: row['cohorte'] as PerfilPrestador['cohort'],
    locationId: String(row['ubicacionId']),
...
function toListing(row: Record<string, unknown>): Publicacion {
  ...
  const storedBookingMode =
    row['modalidadReserva'] === null || row['modalidadReserva'] === undefined
      ? undefined
      : String(row['modalidadReserva'])
  const storedPriceMode =
    row['modalidadPrecio'] === null || row['modalidadPrecio'] === undefined
      ? undefined
      : String(row['modalidadPrecio'])
  const bookingMode = toBookingMode(storedBookingMode, durationMinutes)
  const priceMode = toPriceMode(
    storedPriceMode ?? (storedBookingMode === 'requiere_presupuesto' ? 'presupuesto' : undefined)
  )
  return {
    listingId: String(row['id']),
    contractVersion: String(row['versionContrato']) as Publicacion['contractVersion'],
    tenantId: String(row['tenantId']),
    merchantId: String(row['prestadorId']),
    kind: row['tipo'] as Publicacion['kind'],
    name: String(row['nombre']),
    description: String(row['descripcion']),
    cohort: row['cohorte'] as Publicacion['cohort'],
    locationId: String(row['ubicacionId']),
    currency: String(row['moneda']).toUpperCase(),
    price: Number(priceMinor) / 100,
    priceMinor,
    priceSnapshot: { currency: String(row['moneda']).toUpperCase(), minor: priceMinor },
    availabilityVersion: Number(row['versionDisponibilidad']),
    published: Boolean(row['publicada']),
    policyVersion: String(row['versionPolitica']),
    stock: row['existencias'] === null ? null : Number(row['existencias']),
    durationMinutes,
    capacity: row['capacidad'] === null ? null : Number(row['capacidad']),
    ...(bookingMode === undefined ? {} : { bookingMode }),
    ...(row['duracionEstimadaMinutos'] === null || row['duracionEstimadaMinutos'] === undefined
      ? {}
      : { estimatedDurationMinutes: Number(row['duracionEstimadaMinutos']) }),
    ...(priceMode === undefined ? {} : { priceMode }),
    workingHours: row['horarioTrabajo'] as Publicacion['workingHours'],
    createdAt: new Date(String(row['fechaCreacion'])).toISOString(),
    updatedAt: new Date(String(row['fechaActualizacion'])).toISOString(),
  }
}
...
function toStoredBookingMode(mode: Publicacion['bookingMode']): string | null {
  ...
  if (
    mode === 'visita_diagnostico' ||
    mode === 'duracion_estimada' ||
    mode === 'requiere_presupuesto'
  )
    return mode
  return null
}
...
function toBookingMode(
  value: string | undefined,
  durationMinutes: number | null
): Publicacion['bookingMode'] {
  if (
    value === 'turno_fijo' ||
    value === 'visita_diagnostico' ||
    value === 'duracion_estimada' ||
    value === 'requiere_presupuesto'
  )
    return value as Publicacion['bookingMode']
...
function toCommitment(row: Record<string, unknown>): MarketplaceCommitment {
  ...
  return {
    contractVersion: String(row['versionContrato']) as MarketplaceCommitment['contractVersion'],
    commitmentId: String(row['compromisoId']),
    cartId: String(row['carritoId']),
    tenantId: String(row['tenantId']),
    merchantId: String(row['prestadorId']),
    context: row['contexto'] as MarketplaceCommitment['context'],
    amount: Number(amountMinor) / 100,
    currency,
    status: row['estado'] as MarketplaceCommitment['status'],
    lineIds: Array.isArray(row['idsLineas']) ? row['idsLineas'].map(String) : [],
    ...
    priceSnapshot: {
      currency,
      minor:
        Number(row['cantidad']) > 0 ? amountMinor / BigInt(Number(row['cantidad'])) : amountMinor,
    },
    ...(row['franjaInicio']
      ? { slotStart: new Date(String(row['franjaInicio'])).toISOString() }
      : {}),
    ...(row['franjaFin'] ? { slotEnd: new Date(String(row['franjaFin'])).toISOString() } : {}),
  }
}

function mapearAuditoriaMercadoServicios(
...
function toOutbox(row: Record<string, unknown>): MarketplaceOutboxRecord {
  ...
  const commitmentIds = Array.isArray(payload['commitmentIds'])
    ? payload['commitmentIds'].map(String)
    : undefined
  return {
...
function decodeMarketplaceResponse(value: unknown): MarketplaceCheckoutResponse {
  return JSON.parse(JSON.stringify(value), (key, item) =>
    key === 'minor' && typeof item === 'string' && /^\d+n$/u.test(item)
      ...
      : item
  ) as MarketplaceCheckoutResponse
}
