...
test('WU5 exposes web/PWA POS transport and relational delivery/POS source-of-truth models', () => {
  ...
  assert.match(schema, /model TusDeliveryZone[\s\S]*?tenantId\s+String/)
  assert.match(schema, /model TusDeliveryShift[\s\S]*?zoneId\s+String/)
  assert.match(schema, /model TusDeliveryTask[\s\S]*?commitmentId\s+String/)
  assert.match(schema, /model TusDeliveryProof[\s\S]*?evidenceSource\s+String/)
  assert.match(schema, /model TusDeliveryIncident[\s\S]*?reason\s+String/)
