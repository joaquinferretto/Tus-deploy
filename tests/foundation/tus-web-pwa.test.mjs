...
test('POS refresh checks operation status instead of resubmitting the sale', () => {
  ...
  assert.match(pos, /posOperationStatus/)
  assert.match(pos, /refreshOperationStatus/)
  assert.match(pos, /feedback\.action === 'refresh'[\s\S]{0,160}\? void refreshOperationStatus/)
  assert.doesNotMatch(pos, /onClick=\{\(\) => void sendOperation\(operation\)\}/)
})

test('auth keeps bearer credentials scoped to session storage and never writes persistent local storage', () => {
