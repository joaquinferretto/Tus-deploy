...
test('local API configuration ignores ambient database URLs while production uses platform injection', async () => {
  ...
  try {
    ...
    const production = loadApiRuntimeConfig({
      rootDirectory: root,
      environment: { NODE_ENV: 'production', DATABASE_URL: 'postgresql://platform.example.test/tus' },
    })
    assert.equal(production.databaseUrl, 'postgresql://platform.example.test/tus')
  } finally {
...
test('environment inventory and public domain contracts avoid secret values', () => {
  ...
  assert.match(inventory, /DATABASE_URL` as the only database URL key/u)
  ...
  assert.doesNotMatch(apiExample, /^DATABASE_URL\s*=/mu)
