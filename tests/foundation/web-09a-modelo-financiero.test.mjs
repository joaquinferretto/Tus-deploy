...
const SERVICE_SETUP = `
  ...
  const finance = new ServicioFinanzasServicios(new TransaccionFinanzasServicioEnMemoria(financeStore, new IdentidadServicioEnMemoria(workStore, marketplace)), clock)
...
test('WEB-09A Prisma adapter round-trips bigint amounts, scopes lookups and retries unique races', () => {
  const result = runTypeScriptScenario(`
    ...
    const client = {
      ...
      idempotenciaFinanciera: delegate(tables.idempotenciaFinanciera),
      transactions: 0,
    ...
    console.log(JSON.stringify({ prepared, montoType: typeof persisted.monto, monto: String(persisted.monto), transactions: txAfterPrepare, isolation: client.isolation, providerRead, strangerCode, mappedAmount: String(mapped.amountMinor), idempotency: tables.idempotenciaFinanciera.length }))
  `)
