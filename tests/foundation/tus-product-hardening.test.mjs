...
test('deployment contracts use the canonical local API port and preserve explicit overrides', async () => {
  ...
  const runtimeSource = await readFile(join(root, 'apps/api/src/platform/runtime.ts'), 'utf8')
  ...
  assert.match(runtimeSource, /environment\['API_PORT'\]\s*\?\?\s*environment\['PORT'\]/u)
  assert.match(runtimeSource, /runtimeEnvironment\s*===\s*'production'[\s\S]*environment\['PORT'\]/u)
...
test('Vercel, Next, Render, Mongo, mobile, support, and worker contracts are explicit without secrets', async () => {
  ...
  assert.match(render, /preDeployCommand:\s*node scripts\/deployment\/render-predeploy\.mjs/u)
  assert.match(render, /MONGODB_URL/u)
  assert.doesNotMatch(render, /key:\s*MONGODB_URI/u)
  assert.match(render, /startCommand:\s*python -m worker\.main/u)
  assert.match(render, /WORKER_DEPLOYMENT_STATUS[\s\S]*external-blocked-placeholder/u)
  assert.match(render, /WORKER_ENABLE_CONSUMER[\s\S]*value:\s*false/u)
