...
test('deployment contracts use the canonical local API port and preserve explicit overrides', async () => {
  ...
  const runtime = await readFile(join(root, 'apps/api/src/platform/runtime.ts'), 'utf8')
  ...
  assert.match(runtime, /environment\['API_PORT'\]\s*\?\?\s*environment\['PORT'\]/u)
  assert.match(runtime, /runtimeEnvironment\s*===\s*'production'[\s\S]*environment\['PORT'\]/u)
