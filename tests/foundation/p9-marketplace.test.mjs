...
function runTypeScriptScenario(source) {
  const wrapped = ['(async () => {', 'BigInt.prototype.toJSON = function () { return this.toString() }', source, '})()'].join('\n')
...
test('PR4 publishes durable marketplace facts and emits tenant-scoped audit/outbox records', () => {
  const result = runTypeScriptScenario(`
    ...
    console.log(serializeJson({ published, discovery, audits: store.audit.list('merchant-a'), outbox: store.outbox.list('merchant-a'), contract: validarPublicacionMercadoServicios(published), version: TUS_CONTRACT_VERSION }))
  `)
