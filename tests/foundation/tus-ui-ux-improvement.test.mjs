...
test('PR1 keeps the confirmed bearer credential volatile after tenant scope is returned', () => {
  const result = runTypeScriptScenario(`
    ...
    const client = createTusWebAuthClient({
      transport: { request: async (input) => {
       ...
       storage: { read: () => stored, write: () => { writeCalled = true }, clear: () => { stored = null } },
  ...
  assert.equal(result.state.session.subjectId, 'actor-1')
   ...
   assert.equal(result.writeCalled, false)
  assert.deepEqual(
...
test('PR1 ignores persisted credentials and requires a fresh sign-in', async () => {
  const result = runTypeScriptScenario(`
    const { createTusWebAuthClient } = (await import('./apps/web/src/lib/tus-auth-client.ts')).default
...
test('PR1 maps missing credentials without leaking storage failures', () => {
  const result = runTypeScriptScenario(`
    const { createTusWebAuthClient } = (await import('./apps/web/src/lib/tus-auth-client.ts')).default
  ...
  assert.equal(result.missing.status, 'unauthenticated')
   assert.equal(result.unavailable.status, 'unauthenticated')
