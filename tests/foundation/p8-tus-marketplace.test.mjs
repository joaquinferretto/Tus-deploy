...
test('WU3 persists marketplace tenant boundaries and availability facts in the relational source-of-truth schema', () => {
  ...
  assert.match(schema, /model Prestador[\s\S]*?tenantId\s+String/)
  assert.match(schema, /model Publicacion[\s\S]*?tenantId\s+String/)
  assert.match(schema, /model Publicacion[\s\S]*?versionDisponibilidad\s+Int/)
  assert.match(schema, /model CompromisoMercadoServicios[\s\S]*?tenantId\s+String/)
  assert.match(schema, /model CompromisoMercadoServicios[\s\S]*?contexto\s+String/)
