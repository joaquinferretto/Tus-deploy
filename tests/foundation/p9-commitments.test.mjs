...
test('WU2 adds additive durable commitment, transition, and compensation schema with a rollback-safe migration', () => {
  ...
  assert.match(schema, /model Compromiso[\s\S]*?version\s+Int/)
  assert.match(schema, /model CompensacionCompromiso[\s\S]*?compromisoId\s+String/)
  assert.match(schema, /model TransicionCompromiso[\s\S]*?estadoAnterior\s+String/)
  assert.match(schema, /model ReferenciaAuditoria[\s\S]*?metadatos\s+Json\?/)
