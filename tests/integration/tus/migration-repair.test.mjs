...
test('inventory classifies the complete backlog and excludes comment-only destructive words', async () => {
  ...
  assert.equal(inventory.pendingMigrations.length, directories.length - 5)
  ...
  assert.equal(inventory.migrations.some((migration) => migration.name === POS_INDEX_CONSTRAINT_REPAIR_NAME), true)
  assert.equal(inventory.migrations.some((migration) => migration.name === LIVE_SCHEMA_CONFORMANCE_REPAIR_NAME), true)
  assert.equal(inventory.migrations.some((migration) => migration.name === PHYSICAL_SPANISH_MIGRATION_NAME), true)
  assert.equal(inventory.pendingMigrations.some((migration) => migration.name === POS_INDEX_CONSTRAINT_REPAIR_NAME), false)
  assert.equal(inventory.pendingMigrations.some((migration) => migration.name === LIVE_SCHEMA_CONFORMANCE_REPAIR_NAME), false)
...
test('Prisma launch money fields use BigInt rather than Float', async () => {
  ...
  assert.match(schema, /model MovimientoContable[\s\S]*?monto\s+BigInt/u)
})
...
test('Money JSON uses decimal strings and never leaks bigint serialization errors', () => {
  ...
  assert.throws(() => parseDecimalToMinor('XXX', '1.00'), /verified minor-unit exponent/u)
})
