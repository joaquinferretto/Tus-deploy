...
WEB-08A/B respalda `Trabajo`, `Diagnóstico`, `Presupuesto`, `LineaPresupuesto`, `PresupuestoAceptado` y `EvidenciaTrabajo`
con contracts, persistencia y rutas verificables. No se deben presentar `TusJob`, `TareaEntrega` ni una confirmación WhatsApp
como equivalentes.
...
WEB-08A/B agrega el agregado `Trabajo`, sus transiciones y operaciones HTTP; `ResultadoTrabajo` aún no es un agregado separado. `TusJob` identifica
trabajo técnico de cola y `TareaEntrega` identifica fulfillment de delivery; ambos conservan sus responsabilidades actuales.
...
servicio. El `Presupuesto` WEB-08A/B se relaciona con trabajo, tenant cliente/prestador, importe minor-unit, vigencia y versión;
WEB-08B define y expone su operación HTTP.
...
La evidencia financiera vigente se enlaza a `Compromiso`; WEB-08A agrega `EvidenciaTrabajo` por fase y referencia durable,
sin almacenar binarios.
