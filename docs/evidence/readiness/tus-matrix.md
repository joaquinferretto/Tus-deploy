...
| PostgreSQL authenticated durability | `pnpm test -- tests/integration/tus/postgres-http-smoke.test.mjs` — exit 0; **20 deferred-boundary tests pass**; `TUS_POSTGRES_URL` and `DATABASE_URL` unavailable | Local PostgreSQL HTTP boundary not available in this snapshot | `764d204` working tree / 2026-08-29 | Data and Runtime | Local disposable PostgreSQL, restart/replay, tenant isolation | deferred | `local-postgresql-http` + `deferred` |
