...
future successor is complete. The source folders are inspected read-only when the
configured root and selected evidence files are available. If a root or selected
file is unavailable in a checkout, the harness records that source as
`unavailable/deferred`, skips reads for that source, and makes no external
verification claim. Available selected evidence files remain covered by the
...
The deterministic P1.1 tests classify each configured evidence root as available or
`unavailable/deferred`, verify available roots remain outside the factory and their
selected files are readable without mutation, document provenance without source
listings, and record each regression guardrail. They do not import either source,
run its dependencies, read `.env`, call a cloud provider, or claim external
verification for an unavailable source or that authentication has been implemented.
