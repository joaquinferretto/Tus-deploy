...
- Python/LangGraph as the only orchestration authority; Bedrock as the planned principal AI provider; Groq as the currently active provider until AWS credits exist; retained Groq LLM/STT/TTS adapters; B2 assets; Redis-local and SQS+DLQ production jobs; Docker Compose as the complete integration profile; a Docker-free native local development profile; and two fully supported production profiles: native Render and Terraform-managed AWS.
...
- Production-readiness claims for the native local profile, replacing Compose as the complete integration harness, or requiring provider credentials for local development.
...
- `platform-foundation`: add a separate Docker-free native local profile, explicit optional/fake dependency behavior, root `.env` loading by path/key, canonical `backend`/`frontend` wrappers, and split native-versus-Compose acceptance evidence.

## Approach

Choose a neutral core with always-on platform modules and fully implemented, configurable adapters. This preserves reusable value while removing contamination, avoids an all-in monolith’s coupling and cost, and avoids split repositories’ duplicated governance. Use Clean/Hexagonal boundaries, contract-first cross-runtime schemas, explicit configuration/fail-fast startup, dependency inversion, OTel middleware, deterministic local fakes, and bounded slices. Preserve working mobile, JSON Schema, LangGraph, OTel, and Groq primitives; adapt or rewrite only where contamination, duplication, broken claims, or unsafe behavior prevents neutral reuse.

### Native local development amendment

The native profile is a developer loop, not a replacement for integration. Add source-free canonical wrappers at `backend/` and `frontend/` so the exact commands `cd backend && pnpm run dev` and `cd frontend && pnpm run dev` delegate to `apps/api` and `apps/web` without copying source or secrets. The API wrapper MUST resolve the repository-root `.env` by explicit path, consume the existing `DATABASE_URL` key without printing its value, and start the API with PostgreSQL as the required local dependency. MongoDB and Redis readiness MUST be explicit optional, disabled, or deterministic-fake states under the native profile; other provider-backed capabilities use documented fakes/disabled states. No provider credentials are required.

Native acceptance records separate evidence: both exact wrapper commands run, API/web health succeeds, PostgreSQL connectivity uses `DATABASE_URL`, optional dependencies report their declared fake/disabled state, and logs/scans show no secret values or copied `.env`. Compose remains the complete integration acceptance: `docker compose config` plus full API/web/mobile-support/Python/PostgreSQL/MongoDB/Redis/fakes startup and readiness. Docker unavailability keeps Compose acceptance open.
...
All profiles share contracts, ownership, and rollback semantics, but evidence is profile-specific. Docker Compose remains the complete local integration profile; native local is a PostgreSQL-backed developer profile; production Docker is not used for the Render profile.
...
| Render-native | Render API/web services and Python workers; Neon PostgreSQL; managed MongoDB; Redis; B2 object storage. | Native Render deployment, secret stores, worker lifecycle, and provider-free plus authorized conformance smoke. |
...
| 1. Foundation | Lockfile/namespaces, config validation, API contracts/errors/lifecycle, OTel context, Terraform module boundaries, native local startup, complete Compose integration, both production-profile shapes, CI harness, and split native/Compose evidence. |
...
| Deployment profiles | Native local acceptance and Compose integration acceptance are independently recorded; Render-native and AWS Terraform profiles share behavior and have profile-specific configuration, smoke gates, rollback/runbooks, and no unsupported service claims. |
...
| Operations | Compose runs the full integration profile; native local limitations are explicit; Render-native profiles and runbooks are explicit; secret scans block tracked secrets; production readiness is claimed only after authorized live evidence. |
| Clean-environment portability | A fresh neutral product team/check-out follows the documentation to configure a profile, start the complete local environment, run the neutral reference integrations, verify no vertical contamination, and understand every provider/live gate without hidden maintainer knowledge. |
...
| `infra/terraform`, `openspec/`, CI, Compose, Render/runbooks | Modified | Single-source IaC, both production profiles, local integration, evidence, and security policy. |
...
- **Med:** native local can drift from full integration; keep Compose as the complete profile and require separate acceptance evidence rather than sharing a false readiness claim.
...
- Paid/live implementation is blocked until owners approve regions, budgets, retention, credentials, and adapter conformance; proposal, local fakes, contracts, and planning are not blocked.
