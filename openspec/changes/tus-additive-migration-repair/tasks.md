...
- [ ] 3.2 On only the authorized remote-development target, run bounded apply, verify backup/rollback and abort paths, then run `pnpm test -- tests/integration/tus/migration-repair.test.mjs`; stop on schema, data, ledger, transaction, retry, or cleanup failure. The verifier-only redundant-parentheses correction is implemented and its focused tests pass, but the single subsequent metadata-only verification attempt returned a redacted blocked result; no retry is permitted, so this task remains incomplete.
- [ ] 3.3 After schema proof, rerun durable POS via `tests/integration/tus/postgres-http-smoke.test.mjs` with `applyMigrations: false`: product/service, replay, version conflict, tenant isolation, receipt/audit/outbox, restart, tagged cleanup, provider `0`; write redacted `migration-repair-evidence.md` with five tags and `external-blocked` when incomplete. Deferred because schema verification failed; no seed or POS runtime was invoked.
